...
Definition F := make_mf (fun phi n => phi (n star) = 0 /\ forall m, phi m = 0 -> n star <= m).
...
have leq2: Fpsi star <= m	by apply: (c2 m); have <-: (phi m) = (psi m) by apply (cond m).
have l2: (Fpsi star < m.+1) by trivial.
rewrite -((cond (Fpsi star) l2)) in v2.
have l1: Fphi star < m.+1 by trivial.
rewrite (cond (Fphi star) l1) in v1.
move: (c1 (Fpsi star) v2) (c2 (Fphi star) v1) => ieq1 ieq2.
by elim str; apply/eqP; rewrite eqn_leq; apply/andP.
Qed.
...
Proof.
move => [] G [] ext /choice_cont_ptw_cont cont.
set psi := fun n:nat => 1.
