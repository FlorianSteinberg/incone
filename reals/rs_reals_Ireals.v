...
Require Import all_rs_base rs_reals.
...
Proof.
have mg0: 0 < /2 ^ m by apply /Rinv_0_lt_compat /pow_lt; lra.
have pg0: 0 < /2 ^ p by apply /Rinv_0_lt_compat /pow_lt; lra.
have nlpnm: /2 ^ n <= /2 ^ n + /2 ^ m + /2^ p by lra.
case: I => // Il Iu /andP [/= rIl rIu] /= ineqI.
case: Il rIl ineqI => //Ilm Ile _ ineqI.
case: Iu rIu ineqI => //Ium Iue _ ineqI.
case: J => // Jl Ju /andP [/= rJl rJu]  /= ineqJ.
case: Jl rJl ineqJ => //Jlm Jle _ ineqJ.
case: Ju rJu ineqJ => //Jum Jue _ ineqJ.
case: (BigIntRadix2.mantissa_sign Ium).
	case: (BigIntRadix2.mantissa_sign Jum).
		case: (BigIntRadix2.mantissa_sign Ilm).
			case: (BigIntRadix2.mantissa_sign Jlm).
				by lra.
			rewrite /SFBI2.round_aux.
			admit.
		admit.
	admit.
admit.
...
Definition Rplus_rlzr (phi: names (rep_space_prod rs_R rs_R)) (n: questions rs_R):=
	I.add (nat2p n) (lprj phi n) (rprj phi n).

Lemma Rplus_rec_fun : Rplus \is_recursive_function.
Proof.
exists Rplus_rlzr => phi [x y] [/=[xephin convx] [yephin convy]].
