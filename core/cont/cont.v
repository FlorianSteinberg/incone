...
Lemma cmod_F2MF phi Lf (f: B -> B'): continuity_modulus (F2MF f) phi Lf <->
...
Lemma cntop_F2MF f: continuous_operator (F2MF f) <-> continuous_operator_f f.
Proof.
...
Lemma restr_cntop_w (F: B ->> B') P: F \is_continuous_operator ->
	                            F|_P \is_continuous_operator.
Proof. by move => cont; apply/restr_cntop; first exact/subs_all; rewrite -restr_all. Qed.

Lemma cmod_restr_cntop (F: B ->> B'):
...
Proof. by apply/restr_cntop/cmod_restr_cntop; move => phi []. Qed.

Lemma cntop_sing (F: B ->> B'): F \is_continuous_operator -> F \is_singlevalued.
Proof.
move => /cntop_spec cont; apply/sing_spec_mod => phi phifd.
by have [Lf]:= cont phi phifd; rewrite mod_cmod; exists (LF2MF Lf).
Qed.

Lemma cntop_exte (F G: B ->> B'):
