...
Definition limQ := make_mf(fun xn x =>
...
Lemma limQ_sing: limQ \is_singlevalued.
...
Definition Cauchy_crit_eff := make_subset (fun xn =>
	forall n m, Rabs (xn n - xn m) <= /2^n + /2^m).
...
Definition lim_eff := make_mf (fun xn x => forall n, Rabs (xn n - x) <= /2^n).
...
Lemma lim_eff_sing: lim_eff \is_singlevalued.
Proof.
